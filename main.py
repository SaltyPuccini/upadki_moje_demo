...
def draw_prediction_on_image(image, keypoints_with_scores, crop_region=None, close_figure=False, output_image_height=None):
  ...
  print(time.time() - t0, "aaa")
  t0 = time.time()
  fig, ax = plt.subplots(figsize=(12 * aspect_ratio, 12))
  print(time.time() - t0, "aab")
  t0 = time.time()
...
def image_prediction(image_path):
    ...
    _ = plt.axis('off')
    cv2.imwrite('a.jpg',output_overlay)
...
def gif_prediction(image_path):
    ...
    odleglosc_nos_noga_lewa = []
    odleglosc_nos_noga_prawa = []
    nos_kordy = []
    output_images = []
    ...
    for frame_idx in range(num_frames):
        ...
        odleglosc_nos_noga_lewa.append(keypoints_with_scores[0][0][0][0] - keypoints_with_scores[0][0][15][0])
        odleglosc_nos_noga_prawa.append(keypoints_with_scores[0][0][0][0] - keypoints_with_scores[0][0][16][0])
        nos_kordy.append(keypoints_with_scores[0][0][0][0])

        if len(odleglosc_nos_noga_lewa) == 10:

            if (odleglosc_nos_noga_lewa[0] - odleglosc_nos_noga_lewa[9] >= 0.4 or odleglosc_nos_noga_lewa[0] - odleglosc_nos_noga_lewa[9] <= -0.4) and (nos_kordy[0] - nos_kordy[9] >= 0.1 or nos_kordy[0] - nos_kordy[9] <= -0.1):
                cv2.putText(img=output_images[frame_idx], text='Upadek', org=(80, 150), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1, color=(255, 0, 0), thickness=3)

            if (odleglosc_nos_noga_prawa[0] - odleglosc_nos_noga_prawa[9] >= 0.4 or odleglosc_nos_noga_prawa[0] - odleglosc_nos_noga_prawa[9] <= -0.4) and (nos_kordy[0] - nos_kordy[9] >= 0.1 or nos_kordy[0] - nos_kordy[9] <= -0.1):
                cv2.putText(img=output_images[frame_idx], text='Upadek', org=(80, 150), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=1, color=(255, 0, 0), thickness=3)

            odleglosc_nos_noga_lewa.pop(0)
            odleglosc_nos_noga_prawa.pop(0)
            nos_kordy.pop(0)

    output = np.stack(output_images, axis=0)
    imageio.mimsave('my_very_own_gif.gif', output, fps = 10) # modify the frame duration as needed
